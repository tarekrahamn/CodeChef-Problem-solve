...
void run(){
    ...
    ll maxLength = -1;
    ll numFilteredSongs = 0;
    vll filteredLengths;
     FOR(i,0,N) {
        if (languages[i] == L) {
            filteredLengths.pb(lengths[i]);
            numFilteredSongs++;
        }
    }
    if (numFilteredSongs < K) {
        maxLength = -1; 
    } else {
        sort(des(filteredLengths));
        ll playlistLength = 0;
        FOR(i,0,K) {
            playlistLength += filteredLengths[i];
        }
        maxLength = playlistLength;
    }
    cout << maxLength << endl;
}
int main(){
